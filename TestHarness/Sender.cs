...
namespace CommTest
{
    public class Sender
    {
        ...
        string lastError = "";
        string ToSendPath = "..\\..\\..\\TestHarness\\FileCache";
        string SavePath = "..\\..\\..\\TestHarness\\FileCache";
        int BlockSize = 1024;
        byte[] block;
        void ThreadProc()
        {
       ...
       public bool download(string filename)
        {
            ...
            try
            {
               // hrt.Start();
                ...
                Console.Write("\n  Received file for testing from Repository \"{0}\" of {1} bytes", filename, totalBytes);
            }
            catch (Exception ex)
            {
                 Console.Write("\n  {0}", ex.Message);
